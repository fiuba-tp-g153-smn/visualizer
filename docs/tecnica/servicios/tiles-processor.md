...
`tiles-processor` transforma los datos meteorológicos crudos en teselas WebP, archivos COG y capas
GeoJSON, que deposita en el bucket `tiles-data`. Su arquitectura combina un productor, colas de
RabbitMQ y dos tipos de workers. Debido al tamaño de los archivos geoespaciales, es el componente que
más memoria utiliza. Los productos salen por el almacén de objetos y no por una API HTTP.
...
| Fuente externa (NOAA, ECMWF) | La pasada de esa fuente falla y se registra. Las demás siguen. | No corresponde |
| Un archivo patológico | El subproceso crece hasta el tope de 30 minutos o hasta que el sistema operativo lo mata. No hay límite de memoria declarado. | No corresponde |
