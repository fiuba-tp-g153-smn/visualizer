...
`data-service` publica los productos que genera `tiles-processor`. El sincronizador lee el bucket,
mantiene una copia en Redis y la API responde las solicitudes del navegador. El mismo componente
administra los mapas base y las observaciones de superficie, que no provienen del procesador. Por
este motivo concentra el tráfico de lectura del sistema.
...
El detalle completo está en [12.1 API HTTP](../contratos/api.md). Las familias de rutas:
