...
`data-service`, `alerts-service` y la API de métricas exponen HTTP. El navegador es el único cliente
que cruza los límites entre estos componentes. Las tablas siguientes reproducen las rutas vigentes,
sus parámetros, las respuestas de error y los controles de acceso aplicados.
...
|---|---|---|---|
| `GET` | `/products/{product_id}` | No corresponde | `404` |
| `GET` | `/products/{product_id}/{instrument_id}` | No corresponde | `404` |
...
| Método | Ruta | Errores |
|---|---|---|
| `GET` | `/basemap/providers` | No corresponde |
...
| Método | Ruta | Parámetros | Errores |
|---|---|---|---|
| `GET` | `/weather-stations/latest` | No corresponde | `401`, `503` |
| `GET` | `/weather-stations/tilesets` | No corresponde | `401`, `503` |
| `GET` | `/weather-stations/stations` | No corresponde | `401`, `503` |
...
|---|---|---|
| `GET` | `/metrics/summary` | No corresponde |
| `GET` | `/metrics/sync/status` | No corresponde |
...
| `GET` | `/metrics/redis/memory` | No corresponde |
...
| `GET` | `/metrics/basemap/providers` | No corresponde |
| `GET` | `/sync/status` | No corresponde |
...
|---|---|---|---|---|
| `GET` | `/` | No corresponde | `200` | No corresponde |
| `GET` | `/health` | No corresponde | `200` | No corresponde |
...
| `GET` | `/intersect/layer-refresh-history` | `limit` (20, 1-100) | `200` | No corresponde |
| `POST` | `/alerts` | `{phenomenon_code, geojson}` | `202` con `job_id` | `400` fenómeno inválido, `413` polígono demasiado grande, `503` cola llena |
| `GET` | `/alerts/jobs/{job_id}` | No corresponde | `200` con el estado | `404` |
| `GET` | `/alerts/phenomena` | No corresponde | `200`, 28 entradas | `500` |
| `GET` | `/alerts/limits` | No corresponde | `200` `{max_vertex_count}` | `500` |
| `GET` | `/alerts/pending` | `since_id`, `If-None-Match` | `200` con `ETag`, o `304` | `500` |
| `GET` | `/alerts` | `since_id`, `If-None-Match` | `200` con `ETag`, o `304` | `500` |
| `GET` | `/alerts/{filename}` | No corresponde | `200` GIF | `404` |
| `GET` | `/metrics/summary` | `hours` (24) | `200` | No corresponde |
| `GET` | `/metrics/jobs` | `hours` (24), `limit` (200, hasta 5000) | `200` | No corresponde |
| `GET` | `/metrics/jobs/history` | `hours` (168), `bucket` (`hour`, `day`) | `200` | No corresponde |
| `GET` | `/metrics/processor/history` | `hours` (168) | `200`; lista vacía si las métricas están apagadas | No corresponde |
| `GET` | `/metrics/layers` | `limit` (20, 1-200) | `200` | No corresponde |
...
|---|---|---|---|
| `GET` | `/` | No corresponde | No corresponde |
| `GET` | `/health` | No corresponde | No corresponde |
| `GET` | `/api/summary` | `hours` | No corresponde |
| `GET` | `/api/jobs` | `limit` (50, 0-1000), `offset`, `type`, `outcome`, `hours`, `since`, `before` | No corresponde |
| `GET` | `/api/throughput` | `bucket` (`hour`, `day`, `10min`), `hours` | No corresponde |
| `GET` | `/api/timeseries` | `bucket`, `hours` | No corresponde |
| `GET` | `/api/live` | No corresponde | No corresponde |
| `GET` | `/api/export` | `hours` | No corresponde |
