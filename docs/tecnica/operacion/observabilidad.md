...
Cada backend registra métricas de su propio trabajo y las publica mediante una API. El panel de
estado del visualizador reúne esas respuestas y las representa en el navegador. Esta separación
permite revisar tanto el funcionamiento de cada componente como el recorrido completo de un dato.
...
Con `APP_ENV` distinto de `development`, el servicio de datos y el de avisos emiten sus registros en
el formato JSON de New Relic. Traen la biblioteca del agente como dependencia, sin archivo de
configuración ni arranque del agente: cambia el formato del registro, no sale nada hacia New Relic.
