...
Qué comprueba el sistema por su cuenta antes de que un cambio llegue a producción. Importa para
dimensionar cuánta confianza depositar en el proceso: los cuatro repositorios tienen suite de
pruebas, pero **no todos las hacen valer**, y la diferencia no se nota mirando el estado de la
compuerta.
...
`tiles-processor` no declara esa configuración: sus pruebas **no tienen la red bloqueada**, así que
una prueba puede salir a internet sin que nada lo impida.
...
Ver [CI/CD y despliegue](entrega-continua.md) para el detalle de qué workflow ejecuta cada cosa.
