...
export class LayerConfigService {
          ...
          return forkJoin(periodRequests).pipe(
            map((results) => {
              ...
              const config: EcmwfTileLayerConfig = {
                ...
                availableTilesets: periodsByForecast[forecasts[0]] ?? [],
  ...
  updateEcmwfSelectedForecasts(layerId: string, selectedForecastTimestamps: string[]): void {
    ...
    this.updateConfigMap(layerId, {
      ...config,
      availableTilesets: [...periodSet].sort(),
  ...
  private configsAreEqual(a: LayerConfig, b: LayerConfig): boolean {
    ...
    switch (a.type) {
      case LayerType.TILE: {
        if (a.category !== (b as GoesTileLayerConfig | RadarTileLayerConfig | EcmwfTileLayerConfig).category) return false;
        const bTile = b as GoesTileLayerConfig | RadarTileLayerConfig | EcmwfTileLayerConfig;
