...
export class LayerRenderService {
  ...
  createEcmwfTileLayerForForecastAtTimeIndex(
    ...
    const periodTs = config.availableTilesets[timeIndex];
    if (!periodTs) return null;
  ...
  createEcmwfLayersForPlayback(
    ...
    const currentPeriodTs = config.availableTilesets[currentTimeIndex];
    if (!currentPeriodTs) return result;
      ...
      case LayerType.TILE: {
        ...
        switch (layer.category) {
          ...
          case LayerCategory.ECMWF: {
            ...
            return `${layerId}-[${forecastsKey}]-${tilesets[timeIndex]}`;
          }
          default:
            return layerId;
        }
